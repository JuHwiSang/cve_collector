...
def test_osv_get_by_ghsa_populates_basic_fields():
	with tempfile.TemporaryDirectory() as tmp:
		with DiskCacheAdapter(namespace="test", base_dir=tmp) as cache:
			...
			adapter = OSVAdapter(cache=cache, http_client=_StubHttpJson(payload))
...
def test_osv_get_by_ghsa_uses_cache_when_present():
	with tempfile.TemporaryDirectory() as tmp:
		with DiskCacheAdapter(namespace="test", base_dir=tmp) as cache:
			...
			adapter = OSVAdapter(cache=cache, http_client=_StubHttpJson({}))
			v = adapter.get("GHSA-xxxx-yyyy-zzzz")
			assert v is not None
			assert v.summary == "S"
